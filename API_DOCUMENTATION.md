...
## CORS Support
The API has full CORS support enabled. Preflight `OPTIONS` requests are handled and return a `204 No Content` status with appropriate CORS headers (`Access-Control-Allow-Origin: *`, etc.).
...
* **Add Participant (`POST /api/projects/{id}/participants`)**
  ...
  - **Response (200 OK):**
    ...
    ```
  - **Error Responses:**
    - `404 Not Found`: The email address is not registered inside the application database.
...
* **Update Task (`PUT /api/tasks/{id}`)**
  ...
  - **Validation Rules:**
    - `status` (if provided) must be one of: `todo`, `in_progress`, `done` (returns `422 Unprocessable Entity` otherwise).
    - `assigned_to_id` (if provided) must refer to a registered user who is currently a Participant or the Owner of the task's project (returns `422 Unprocessable Entity` otherwise).
